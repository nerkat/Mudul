  ...
  await new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      db.run(
        ...
        ['mem-1', 'user-1', 'acme', 'OWNER']
      );

      db.run(
        'INSERT INTO memberships (id, user_id, org_id, role) VALUES (?, ?, ?, ?)',
        ['mem-2', 'user-2', 'acme', 'VIEWER']
      );

      // Insert clients
      db.run(
        'INSERT INTO clients (id, org_id, name, slug) VALUES (?, ?, ?, ?)',
        ['client-acme', 'acme', 'Acme Corp', 'acme-corp']
      );

      db.run(
        'INSERT INTO clients (id, org_id, name, slug) VALUES (?, ?, ?, ?)',
        ['client-beta', 'acme', 'Beta Systems', 'beta-systems']
      );

      db.run(
        'INSERT INTO clients (id, org_id, name, slug) VALUES (?, ?, ?, ?)',
        ['client-gamma', 'acme', 'Gamma Industries', 'gamma-industries']
      );

      // Insert calls
      db.run(
        'INSERT INTO calls (id, org_id, client_id, name, summary, ts, duration_sec, sentiment, score, booking_likelihood) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ['call-acme-1', 'acme', 'client-acme', 'Discovery Call - Jan 15', 
      ...
      db.run(
        'INSERT INTO calls (id, org_id, client_id, name, summary, ts, duration_sec, sentiment, score, booking_likelihood) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ['call-acme-2', 'acme', 'client-acme', 'Follow-up Call - Jan 20',
      ...
      db.run(
        'INSERT INTO calls (id, org_id, client_id, name, summary, ts, duration_sec, sentiment, score, booking_likelihood) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ['call-beta-1', 'acme', 'client-beta', 'Initial Contact - Jan 12',
      ...
      db.run(
        'INSERT INTO calls (id, org_id, client_id, name, summary, ts, duration_sec, sentiment, score, booking_likelihood) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ['call-beta-2', 'acme', 'client-beta', 'Demo Call - Jan 18',
      ...
      db.run(
        'INSERT INTO calls (id, org_id, client_id, name, summary, ts, duration_sec, sentiment, score, booking_likelihood) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ['call-gamma-1', 'acme', 'client-gamma', 'Qualification Call - Jan 22',
      ...
      db.run(
        ...
        ['action-1', 'acme', 'client-acme', 'user-1', 'Send follow-up proposal to Acme Corp', '2024-01-25T00:00:00Z', 'OPEN']
      );

      db.run(
        'INSERT INTO action_items (id, org_id, client_id, owner_id, text, due, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ['action-2', 'acme', 'client-beta', 'user-1', 'Schedule demo for Beta Systems', '2024-01-20T00:00:00Z', 'DONE']
      );

      db.run(
        'INSERT INTO action_items (id, org_id, client_id, owner_id, text, due, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ['action-3', 'acme', 'client-gamma', 'user-1', 'Research Gamma Industries requirements', '2024-01-30T00:00:00Z', 'OPEN']
      );

      db.run(
        'INSERT INTO action_items (id, org_id, client_id, text, status) VALUES (?, ?, ?, ?, ?)',
        ['action-4', 'acme', 'client-acme', 'Contact technical team for integration details', 'OPEN'],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
